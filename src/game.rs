...
impl Game {
                ...
                if let Some(command) = self.get_command(interval - now.elapsed()) {
                    match command {
                        ...
                        Command::Adjust(towards) => {
                            ...
                            else if Speed::Up == towards && self.speed != 20{
                                self.speed += 1;
                            }
                        }
                    }
    ...
    fn get_command(&self, wait_for: Duration) -> Option<Command>{
        ...
        match key_event.code {
            ...
            KeyCode::Char('u') => Some(Command::Adjust(Speed::Up)),
    ...
    pub fn has_collided_with_wall(&mut self) -> bool{
    ...
    pub fn has_bitten_itself(&mut self) -> bool{
    ...
    fn draw_speed(&mut self) {
        let speed = self.speed;
        if self.speed < 10 {
            self.stdout
            .execute(MoveTo(0, self.height + 4)).unwrap()
            .execute(Print("Speed: ")).unwrap()
            .execute(Print(speed.to_string())).unwrap()
            .execute(Print(" ")).unwrap();
        }
        self.stdout
        .execute(MoveTo(0, self.height + 4)).unwrap()
    ...
    fn draw_food(&mut self) {
        self.stdout.execute(SetForegroundColor(Color::Black)).unwrap();
